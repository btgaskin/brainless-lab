...
| `:falandays_dendritic` | experimental | + a **parallel dendritic pathway**: each node's recurrent synapses are split across `n_dendrites` compartments (default 4); each dendrite integrates its own routed current and fires a local **dendritic spike** that sets an **eligibility tag** on its synapses. The homeostatic weight update then gates on *presynaptic-spike **or** dendrite-tag*, so a dendritic event can license learning with no somatic/presynaptic spike -- while the soma still receives the full recurrent sum. Endogenous **logistic** drive (dendrite and/or soma). A discrete, homeostatic caricature of dendritic computation, distinct from the biophysical `:compartmental_*` cells. Port of `v0.2/crho/node_dendritic.py`. |
