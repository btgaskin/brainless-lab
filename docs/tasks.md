...
| `:pong` | **46** | bearing from paddle to ball over `-90:4:90 deg`; the matching angular bin is active when the ball is in front of the paddle | **2** | paddle vote/differential command: `paddle_y += 100 * (e1 - e2)`, clamped to the paddle range | `mean_align` (paddle tracks ball); floor ~= 0.33 |
| `:pong_hitrate` | **46** | same env and sensors as `:pong` | **2** | same paddle command as `:pong` | `hit_rate`, a distinct scoring of the same env |
