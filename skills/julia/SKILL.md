...
1. **Can the compiler infer a concrete type for every variable?** ("type stability")
2. **Does this code allocate memory it doesn't need to?** ("allocations")

Type instability causes allocations (the compiler can't put an unpredictably-typed value in a
register or on the stack, so it boxes it on the heap). Allocations trigger the garbage collector,
which stops the world while it runs. Almost every other rule in this skill — avoid untyped
globals, write functions not scripts, prefer dispatch over branching, use views, preallocate
buffers — is a *consequence* of these two facts, not an independent rule to memorize. When you
hit a Julia performance or "why is this weird" problem that isn't covered explicitly below, ask
"is something here not type-stable?" before "is something here not parallel enough?" — instability
is the more common root cause by a wide margin, and fixing it is what unlocks the rest.
...
- **Mutate in place in hot loops** (functions ending in `!`, e.g. `du .= ...`) instead of
  allocating a new array on every iteration or every call.
...
| A `struct` field typed `Real`, `Number`, `AbstractVector`, or left untyped | Field type isn't concrete → boxing on every access | `type-stability.md` |
| `v = []` then `push!`-ing into it | Infers as `Vector{Any}`; every element boxed | `type-stability.md` |
| A function whose return type depends on a *runtime value*, not argument types | Classic type instability; `findfirst`-style `T`-or-`nothing` is the canonical example | `type-stability.md` |
...
| An ODE/PDE right-hand-side function returning a freshly allocated array instead of writing in-place (no `!`, no mutated first argument) | Forces the solver to allocate every single step | `scientific-ecosystem.md` |
