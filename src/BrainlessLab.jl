...
include("analysis/ProbeDecodability.jl")
...
register_task!(:delayed_cue, DELAYED_CUE_TASK)
for (name, options) in pairs(CAPACITY_PROBE_DEFAULTS)
    register_task!(name, _capacity_task(name, options))
end
...
register_analysis!(:probe_decodability, probe_decodability;
    label="Fixed-wiring probe decoding (experimental)", scope=:block,
    required_channels=(:probe_events,), options=PROBE_DECODER_OPTIONS)
