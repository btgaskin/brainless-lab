...
function sweepable_axes(node=:falandays_base, task=:wall)
    ...
    elseif _is_compartmental_node(node_sym)
        ...
        push!(out, SweepAxisInfo(path="node.substeps", default=5, range=">= 1", description="forward-Euler sub-integration steps per env step (temporal window K)"))
    ...
    if is_multiagent(task_obj.setup)
        ...
        push!(out, SweepAxisInfo(path="env.vision_range", default=cfg.vision_range, range="nothing or > 0", description="maximum conspecific vision distance; source fallback"))
        push!(out, SweepAxisInfo(path="env.source_vision_range", default=cfg.source_vision_range, range="nothing or > 0", description="source vision range; nothing => uses vision_range"))
