...
BrainlessLab.sense!(::_MixedBodyA, percept) = Float64[percept[1]]
BrainlessLab.sense!(::_MixedBodyB, percept) = Float64[percept[1], percept[2]]
BrainlessLab.decode!(::_MixedBodyA, values) = Vector{Float64}(values)
BrainlessLab.decode!(::_MixedBodyB, values) = Vector{Float64}(values)
BrainlessLab.component_state(::_MixedBodyB) = (death=(tick=3, cause=:test),)
...
BrainlessLab.sense!(::_DyingBody, percept) = Float64[percept[1]]
BrainlessLab.decode!(::_DyingBody, values) = Vector{Float64}(values)
BrainlessLab.alive(body::_DyingBody) = body.is_alive
BrainlessLab.update!(body::_DyingBody, effects=()) = (body.is_alive = false; nothing)
...
function BrainlessLab.sample!(environment::_MixedEnvironment, bodies)
    percepts = [fill(environment.state, n_receptors(body)) for body in bodies]
    push!(environment.samples, deepcopy(percepts))
    return percepts
end

function BrainlessLab.apply_commands!(environment::_MixedEnvironment, bodies, commands)
...
function BrainlessLab.sense!(body::_TypedDyingBody, percept)
    body.sense_calls += 1
    return Float64[percept[1]]
end
BrainlessLab.decode!(::_TypedDyingBody, values) = _TypedCommand(Float64(only(values)))
BrainlessLab.inactive_command(::_TypedDyingBody) = _TypedCommand(0.0)
...
function BrainlessLab.apply_commands!(environment::_TypedEnvironment, bodies, commands)
...
for T in (_CountingDyingBodyA, _CountingDyingBodyB)
    @eval begin
        n_receptors(::$T) = 1
        n_effectors(::$T) = 1
        function BrainlessLab.sense!(body::$T, percept)
            body.sense_calls += 1
            return Float64[percept[1]]
        end
        BrainlessLab.decode!(::$T, values) = Vector{Float64}(values)
        BrainlessLab.alive(body::$T) = body.is_alive
        BrainlessLab.update!(body::$T, effects=()) = (body.is_alive = false; nothing)
    end
end
...
function BrainlessLab.sense!(body::_BorrowingBody, percept)
...
function _mixed_agents()
    ...
    body_b = _MixedBodyB()
    body_a2 = _MixedBodyA()
    return BrainlessLab.Agent[
        BrainlessLab.Agent(_MixedReservoir(1, 1), body_a1),
        BrainlessLab.Agent(_MixedReservoir(2, 1), body_b),
        BrainlessLab.Agent(_MixedReservoir(1, 1), body_a2),
    ]
end

@testset "homogeneous and grouped stores" begin
    homogeneous_agents = _mixed_agents()[[1, 3]]
    homogeneous = BrainlessLab.Ensemble(homogeneous_agents, _MixedEnvironment())
    @test homogeneous.store isa BrainlessLab.HomogeneousStore
    @test BrainlessLab.nagents(homogeneous) == 2
    @test BrainlessLab.entity_ids(homogeneous) == BrainlessLab.EntityID.(1:2)

    mixed = BrainlessLab.Ensemble(_mixed_agents(), _MixedEnvironment(); ids=[11, 22, 33])
    @test mixed.store isa BrainlessLab.GroupedStore
    @test BrainlessLab.nagents(mixed) == 3
    @test BrainlessLab.entity_ids(mixed) == BrainlessLab.EntityID[BrainlessLab.EntityID(11), BrainlessLab.EntityID(22), BrainlessLab.EntityID(33)]
    @test BrainlessLab.body_at_slot(mixed, 1) isa _MixedBodyA
    @test BrainlessLab.body_at_slot(mixed, 2) isa _MixedBodyB
    @test BrainlessLab.body_at_slot(mixed, 3) isa _MixedBodyA
    @test BrainlessLab.agent_at_slot(mixed, 2).body isa _MixedBodyB
    @test length(mixed.store.groups) == 2
    @test BrainlessLab.group_slots(mixed.store.groups[1]) == [1, 3]
    @test BrainlessLab.group_slots(mixed.store.groups[2]) == [2]
    @test BrainlessLab.group_ids(mixed.store.groups[1]) == BrainlessLab.EntityID[BrainlessLab.EntityID(11), BrainlessLab.EntityID(33)]
    exposed_slots = BrainlessLab.group_slots(mixed.store.groups[1])
    exposed_slots[1] = 99
    @test BrainlessLab.group_slots(mixed.store.groups[1]) == [1, 3]
    exposed_agents = BrainlessLab.group_agents(mixed.store.groups[1])
    pop!(exposed_agents)
    @test length(BrainlessLab.group_agents(mixed.store.groups[1])) == 2
    sizes = Int[]
    BrainlessLab.foreach_group(group -> push!(sizes, length(BrainlessLab.group_agents(group))), mixed)
    @test sizes == [2, 1]
    @test_throws ArgumentError BrainlessLab.Ensemble(_mixed_agents(), _MixedEnvironment(); ids=[1, 1, 2])
    mismatched = BrainlessLab.Agent(_MixedReservoir(2, 1), _MixedBodyA())
    @test_throws DimensionMismatch BrainlessLab.Ensemble([mismatched], _MixedEnvironment())
end

@testset "mixed stepping remains synchronous" begin
    environment = _MixedEnvironment()
    ensemble = BrainlessLab.Ensemble(_mixed_agents(), environment; ids=[101, 202, 303])
...
@testset "environment metrics are public and optional" begin
    environment = _MixedEnvironment()
    result = BrainlessLab.rollout!(BrainlessLab.Ensemble(_mixed_agents(), environment), 2; window=2)
    @test result.state == environment.state

    metricless = _MetriclessEnvironment()
    @test metrics(metricless, 2) == NamedTuple()
    fallback_result = BrainlessLab.rollout!(
        BrainlessLab.Ensemble([BrainlessLab.Agent(_MixedReservoir(1, 1), _MixedBodyA())], metricless),
        2;
        window=2,
    )
...
@testset "custom typed commands and inactive sensing use public contracts" begin
    ...
    ensemble = BrainlessLab.Ensemble([BrainlessLab.Agent(_MixedReservoir(1, 1), body)], environment)
...
@testset "inactive sensing is skipped through grouped function barriers" begin
    ...
    ensemble = BrainlessLab.Ensemble(
        BrainlessLab.Agent[
            BrainlessLab.Agent(_MixedReservoir(1, 1), body_a),
            BrainlessLab.Agent(_MixedReservoir(1, 1), body_b),
        ],
...
@testset "borrowed receptor buffers are copied at the gather boundary" begin
    ...
    ensemble = BrainlessLab.Ensemble(
        [
            BrainlessLab.Agent(first_reservoir, shared_body),
            BrainlessLab.Agent(second_reservoir, shared_body),
        ],
        _SlotEnvironment(),
    )
...
@testset "concrete runtime paths remain inferred with bounded allocations" begin
    homogeneous = BrainlessLab.Ensemble(_mixed_agents()[[1, 3]], _MixedEnvironment())
    grouped = BrainlessLab.Ensemble(_mixed_agents(), _MixedEnvironment())
...
@testset "entity-aware recording and manifests" begin
    sparse = BrainlessLab.EntityFrame([42, 7], ["forty-two", "seven"])
    @test BrainlessLab.entity_index(sparse, 42) == 1
    @test BrainlessLab.entity_value(sparse, BrainlessLab.EntityID(7)) == "seven"
    aligned = BrainlessLab.align_entities(sparse, [7, 42])
    @test aligned.ids == BrainlessLab.EntityID.([7, 42])
    @test aligned.values == ["seven", "forty-two"]
    @test_throws ArgumentError BrainlessLab.EntityFrame([7, 7], [1, 2])
    @test_throws ArgumentError BrainlessLab.align_entities(sparse, [7, 8])

    recorder = Recorder(enabled=(:spikes, :receptors, :effectors, :body_alive, :deaths))
    ensemble = BrainlessLab.Ensemble(_mixed_agents(), _MixedEnvironment(); ids=[7, 8, 9], recorder=recorder)
    step!(ensemble)

    for channel in (:spikes, :receptors, :effectors, :body_alive, :deaths)
        frame = only(getchannel(recorder, channel))
        @test frame isa BrainlessLab.EntityFrame
        @test frame.ids == BrainlessLab.EntityID[BrainlessLab.EntityID(7), BrainlessLab.EntityID(8), BrainlessLab.EntityID(9)]
        @test length(frame) == 3
    end
    ...
    @test Tuple(agent.id for agent in config.agents) == Tuple(BrainlessLab.EntityID.((7, 8, 9)))
    @test Tuple(agent.slot for agent in config.agents) == (1, 2, 3)
    @test config.entity_ids == Tuple(BrainlessLab.EntityID.((7, 8, 9)))
...
@testset "task setups expose per-body port layouts" begin
    ...
    layouts = BrainlessLab.resolved_task_ports(task)
