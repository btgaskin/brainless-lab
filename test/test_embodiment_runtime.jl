...
function _runtime_sensor_sample(sensor, state, tick, arena)
    if sensor isa BrainlessLab.SpectralCamera
    ...
    elseif sensor isa BrainlessLab.MountedFieldProbe
        field = BrainlessLab.LinearSpatialField((0.0, 0.0), (0.0, 1.0); offset=0.0, scale=10.0)
        return BrainlessLab.sample!(sensor, field, state.position, state.heading, tick, arena)
    end
...
function _runtime_sampler(body, state, tick, arena)
    samples = Tuple(
        ...
        for sensor in BrainlessLab.sensor_components(body)
    )
...
@testset "port contracts reject invalid counts and ambiguous identities" begin
    @test_throws ArgumentError BrainlessLab.PortSpec(-1, 0)
    @test_throws ArgumentError BrainlessLab.PortSpec(0, -1)
    @test_throws ArgumentError BrainlessLab.PortSpec(
        2,
        0,
        [BrainlessLab.Port(:duplicate), BrainlessLab.Port(:duplicate)],
        typeof(BrainlessLab.Port(:unused))[],
    )
    @test_throws ArgumentError BrainlessLab.PortSpec(
        1,
        1,
        [BrainlessLab.Port(:shared)],
        [BrainlessLab.Port(:shared)],
    )
end
...
@testset "configured embodiments run through the main physical loop" begin
    cases = (
        (:differential_robot, 72, 2, [1.0, 0.0], BrainlessLab.DifferentialDriveCommand),
        (:planar_uav, 97, 3, [1.0, 0.5, 1.0], BrainlessLab.PlanarForceYawCommand),
        (:bilateral_insect, 3, 2, [1.0, 0.75], BrainlessLab.ForwardTurnCommand),
    )
    ...
    for (name, nr, ne, output, command_type) in cases
        ...
        @test length(unique(port.id for port in BrainlessLab.ports(body).receptors)) == nr
        @test all(slot -> BrainlessLab.component_id(slot) isa Symbol, BrainlessLab.component_slots(body).sensors)
        if name === :planar_uav
            @test length(BrainlessLab.encoder_components(body)) == 2
            @test BrainlessLab.encoder_sources(BrainlessLab.encoder_components(body)[1]) ==
                  (:antenna_left, :antenna_right)
            @test BrainlessLab.encoder_sources(BrainlessLab.encoder_components(body)[2]) == (:camera,)
            @test BrainlessLab.component_id(BrainlessLab.component_slots(body).encoders[2]) ===
                  :camera__identity_encoder
        end

        state = BrainlessLab.MotionState2D(position=(5.0, 5.0), heading=0.0)
        environment = BrainlessLab.EmbodiedEnvironment(BrainlessLab.WalledArena(20.0), [state], _runtime_sampler)
        raw = only(BrainlessLab.sample!(environment, [body]))
        encoded = BrainlessLab.sense!(body, raw)
        @test BrainlessLab.sense!(body, raw) === encoded
        ...
        command = BrainlessLab.decode!(body, output)
        @test command isa command_type
        @test BrainlessLab.decode!(body, output) === command

        reservoir = _PresetReservoir(nr, output)
        ensemble = BrainlessLab.Ensemble([BrainlessLab.Agent(reservoir, body)], environment)
...
@testset "inactive embodiments do not integrate inertial motion" begin
    ...
    effects = BrainlessLab.apply_commands!(environment, [body], [BrainlessLab.ForwardTurnCommand(0.8, 1.2)])
...
@testset "embodied activity owns reset and active-only motion metrics" begin
    ...
    environment = BrainlessLab.EmbodiedEnvironment(
        BrainlessLab.WalledArena(20.0),
        [
            BrainlessLab.MotionState2D(position=(3.0, 4.0), velocity=(3.0, 4.0)),
            BrainlessLab.MotionState2D(position=(7.0, 8.0), velocity=(8.0, 6.0)),
        ],
        _runtime_sampler,
    )
    BrainlessLab.sync_activity!(environment, [active_body, inactive_body])
...
@testset "embodiment runtime configuration retains the component graph" begin
    ...
    @test length(only(config.sensors).wavelengths_nm) == length(BrainlessLab.DEFAULT_CAMERA_WAVELENGTHS_NM)
...
@testset "component state records by stable entity and component IDs" begin
    body = _preset_body(:bilateral_insect)
    state = BrainlessLab.MotionState2D(position=(5.0, 5.0))
    environment = BrainlessLab.EmbodiedEnvironment(BrainlessLab.WalledArena(20.0), [state], _runtime_sampler)
    reservoir = _PresetReservoir(n_receptors(body), [1.0, 0.75])
    recorder = Recorder(enabled=(:components,))
    ensemble = BrainlessLab.Ensemble([BrainlessLab.Agent(reservoir, body)], environment; ids=[BrainlessLab.EntityID(41)], recorder=recorder)
    step!(ensemble)
    frame = only(getchannel(recorder, :components))
    @test frame isa BrainlessLab.EntityFrame
    @test frame.ids == [BrainlessLab.EntityID(41)]
...
@testset "inactive embodiments do not advance stateful sensors" begin
    body = _preset_body(:bilateral_insect)
    left = BrainlessLab.component_value(first(BrainlessLab.component_slots(body).sensors))
    before = copy(left.state.values)
    body.physiology.is_alive = false
    environment = BrainlessLab.EmbodiedEnvironment(
        BrainlessLab.WalledArena(20.0),
        [BrainlessLab.MotionState2D(position=(5.0, 5.0))],
        _runtime_sampler,
    )
    percept = only(BrainlessLab.sample!(environment, [body]))
    @test all(sample -> all(iszero, sample), percept)
    @test left.state.values == before
    @test all(iszero, BrainlessLab.sense!(body, percept))
end
...
@testset "embodiment reset and effect policy are explicit" begin
    ...
    state = BrainlessLab.MotionState2D(position=(5.0, 5.0))
    environment = BrainlessLab.EmbodiedEnvironment(BrainlessLab.WalledArena(20.0), [state], _runtime_sampler)
    BrainlessLab.sense!(body, only(BrainlessLab.sample!(environment, [body])))
    @test any(!iszero, BrainlessLab.component_value(first(BrainlessLab.component_slots(body).sensors)).state.values)
    reset!(body)
    @test all(iszero, BrainlessLab.component_value(first(BrainlessLab.component_slots(body).sensors)).state.values)

    user_state = _ResettableBodyState(3)
    stateful = BrainlessLab.Embodiment(state=user_state)
    reset!(stateful)
    @test user_state.value == 0

    @test_throws ArgumentError BrainlessLab.update!(body, (:unhandled,))
    ignoring = BrainlessLab.direct_embodiment(
        1,
        1;
        physiology=BrainlessLab.NoPhysiology(unknown_effects=BrainlessLab.IgnoreUnknownEffects()),
    )
    @test BrainlessLab.update!(ignoring, (:unhandled,)) === nothing
end
