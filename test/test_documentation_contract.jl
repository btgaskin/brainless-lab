...
@testset "maintained documentation has one current route set" begin
    ...
    @test isfile(joinpath(docs, "benchmarks", "tasks", "delayed-cue.mdx"))
    @test isfile(joinpath(repository, "benchmarks", "direct-control", "v2", "README.md"))
    @test isfile(joinpath(repository, "experiments", "direct-control", "v2", "experiment.toml"))
