...
target = EvaluationTarget(
    ...
    composition,
    evaluation;
    interventions=(ScheduledIntervention(1800, :freeze_plasticity),),
)
```
...
Versions 1 and 2 remain readable where their schemas apply. The current writer emits
version 3. Version 2 introduced the public evolution schema. Version 3 records the
composition interface, topology profile key, sweep-axis scope, scheduled target
interventions, registered analysis options, compute strides, and aggregate analysis-series
tables.
...
New plans should declare `format_version = 3`. Legacy version 1 plans do not accept an
`[evolve]` section. Version 3 also records the composition interface, topology profile key,
sweep-axis scope, scheduled interventions, and configurable profile analyses.
...
Evolution is experimental software and is not part of the current introductory or
benchmark paths. A completed search and selected model do not establish a scientific
claim.
