...
Evolution.search_strategy(DEFAULT_REGISTRY, :sepcma)
```
...
The integrated fixed node designs are `StructuredCompartmental` and
`DenseCompartmental`, registered as `:compartmental_structured` and
`:compartmental_dense`. Search changes model coordinates only. It does not evolve topology,
node count, body structure, or ports.
